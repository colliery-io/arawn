...
> Generated: 2026-05-13T04:09:10Z | 289 files | Python, Rust
...
-  `ExtractorBindHook` struct L634-637 — `{ runner: Arc<arawn_extractor::ExtractorRunner>, store: Arc<std::sync::Mutex<ara...`
-  `ExtractorBindHook` type L638-672 — `= ExtractorBindHook`
-  `on_bind` function L639-671 — `(&self, workstream_name: &str, feed_id: &str)`
-  `run_cli_via_server` function L1100-1205 — `( url: &str, prompt: &str, session_id: Option<Uuid>, ) -> Result<()>` — Run a CLI prompt by connecting to the running server via WebSocket.
-  `build_llm_client` function L1208-1231 — `( config: &arawn_bin::LlmConfig, ) -> Result<Arc<dyn arawn_llm::LlmClient>>` — Build the appropriate LLM client based on provider config.
-  `register_default_tools` function L1234-1280 — `( registry: &Arc<arawn_engine::ToolRegistry>, config: &arawn_bin::ArawnConfig, d...` — Register all default tools into the registry.
-  `connect_mcp_servers` function L1283-1331 — `( data_dir: &str, plugin_result: &arawn_engine::plugins::PluginLoadResult, regis...` — Connect to MCP servers from config and plugins.
-  `register_workflow_tools` function L1334-1351 — `( registry: &Arc<arawn_engine::ToolRegistry>, workflows_dir: std::path::PathBuf,...` — Register workflow management tools.
-  `build_engine_config` function L1353-1388 — `( config: &arawn_bin::ArawnConfig, workstream: &arawn_core::Workstream, data_dir...`
-  `dirs_path` function L1390-1399 — `() -> Option<String>`
...
- pub `error` module L22 — `-` — The steward continuously re-reads each workstream's KB and applies
- pub `journal` module L23 — `-` — T-0259 wires the /workstream refine / journal / rollback commands.
- pub `runner` module L24 — `-` — T-0259 wires the /workstream refine / journal / rollback commands.
- pub `subroutine` module L25 — `-` — T-0259 wires the /workstream refine / journal / rollback commands.
