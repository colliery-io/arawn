...
> Generated: 2026-05-08T20:25:45Z | 229 files | Python, Rust
...
- pub `default_registry` function L13-20 — `() -> FeedTemplateRegistry` — Build the registry of every template the binary supports.
...
-  `MockClients` type L153-157 — `impl FeedClients for MockClients` — every Slack-touching template test will reuse.
-  `slack` function L154-156 — `(&self) -> Option<Arc<dyn SlackFeedClient>>` — every Slack-touching template test will reuse.
-  `slack_msg` function L159-166 — `(ts: &str, text: &str) -> Value` — every Slack-touching template test will reuse.
-  `read_jsonl` function L170-180 — `(feed_dir: &PathBuf, day: &str) -> Vec<Value>` — Walk a YYYY-MM-DD.jsonl file in `feed_dir` and return all parsed
-  `run_once` function L182-208 — `( template: &dyn FeedTemplate, ctx: &TemplateCtx, params: &TemplateParams, feed_...` — every Slack-touching template test will reuse.
-  `first_run_writes_messages_and_advances_cursor` function L211-259 — `()` — every Slack-touching template test will reuse.
-  `second_run_passes_cursor_and_only_writes_new` function L262-308 — `()` — every Slack-touching template test will reuse.
-  `empty_run_is_a_no_op_with_status` function L311-344 — `()` — every Slack-touching template test will reuse.
-  `messages_partition_across_days` function L347-385 — `()` — every Slack-touching template test will reuse.
-  `run_returns_auth_when_slack_not_connected` function L388-411 — `()` — every Slack-touching template test will reuse.
-  `NoSlack` struct L389 — `-` — every Slack-touching template test will reuse.
-  `NoSlack` type L390-394 — `impl FeedClients for NoSlack` — every Slack-touching template test will reuse.
-  `slack` function L391-393 — `(&self) -> Option<Arc<dyn SlackFeedClient>>` — every Slack-touching template test will reuse.
-  `slack_msg_with_replies` function L415-423 — `(ts: &str, text: &str, reply_count: u64) -> Value` — every Slack-touching template test will reuse.
-  `parent_with_replies_seeds_thread_file_and_advances_thread_cursor` function L426-498 — `()` — every Slack-touching template test will reuse.
-  `second_run_advances_thread_cursor_independently` function L501-566 — `()` — every Slack-touching template test will reuse.
-  `channel_archive_works_for_dm_id_passthrough` function L569-601 — `()` — every Slack-touching template test will reuse.
-  `channel_archive_works_for_mpim_id_passthrough` function L604-631 — `()` — every Slack-touching template test will reuse.
-  `classify_helper_resolves_kinds_for_picker_use` function L634-648 — `()` — every Slack-touching template test will reuse.
-  `thread_failure_does_not_block_channel_or_other_threads` function L651-715 — `()` — every Slack-touching template test will reuse.
...
-  `MockClients` type L111-115 — `impl FeedClients for MockClients` — channel-archive already exercises.
-  `slack` function L112-114 — `(&self) -> Option<Arc<dyn SlackFeedClient>>` — channel-archive already exercises.
-  `dm_msg` function L117-124 — `(ts: &str, text: &str) -> Value` — channel-archive already exercises.
-  `read_jsonl` function L126-136 — `(feed_dir: &PathBuf, day: &str) -> Vec<Value>` — channel-archive already exercises.
-  `run_once` function L138-163 — `( template: &dyn FeedTemplate, ctx: &TemplateCtx, params: &TemplateParams, feed_...` — channel-archive already exercises.
-  `dm_archive_opens_dm_then_writes_messages` function L166-212 — `()` — channel-archive already exercises.
-  `dm_archive_returns_auth_when_slack_not_connected` function L215-238 — `()` — channel-archive already exercises.
-  `NoSlack` struct L216 — `-` — channel-archive already exercises.
-  `NoSlack` type L217-221 — `impl FeedClients for NoSlack` — channel-archive already exercises.
-  `slack` function L218-220 — `(&self) -> Option<Arc<dyn SlackFeedClient>>` — channel-archive already exercises.
...
-  `MockClients` type L105-109 — `impl FeedClients for MockClients` — - Empty result writes nothing and reports `no-new-items`.
-  `slack` function L106-108 — `(&self) -> Option<Arc<dyn SlackFeedClient>>` — - Empty result writes nothing and reports `no-new-items`.
-  `mention_msg` function L111-120 — `(ts: &str, channel: &str, text: &str) -> Value` — - Empty result writes nothing and reports `no-new-items`.
-  `read_jsonl` function L122-133 — `(feed_dir: &PathBuf, day: &str) -> Vec<Value>` — - Empty result writes nothing and reports `no-new-items`.
-  `run_once` function L135-160 — `( template: &dyn FeedTemplate, ctx: &TemplateCtx, params: &TemplateParams, feed_...` — - Empty result writes nothing and reports `no-new-items`.
-  `first_run_resolves_user_id_and_writes_mentions` function L163-208 — `()` — - Empty result writes nothing and reports `no-new-items`.
-  `second_run_uses_cached_user_id_and_dedupes_overlap` function L211-268 — `()` — - Empty result writes nothing and reports `no-new-items`.
-  `empty_run_is_a_no_op` function L271-301 — `()` — - Empty result writes nothing and reports `no-new-items`.
-  `returns_auth_when_slack_not_connected` function L304-325 — `()` — - Empty result writes nothing and reports `no-new-items`.
-  `NoSlack` struct L305 — `-` — - Empty result writes nothing and reports `no-new-items`.
-  `NoSlack` type L306-310 — `impl FeedClients for NoSlack` — - Empty result writes nothing and reports `no-new-items`.
-  `slack` function L307-309 — `(&self) -> Option<Arc<dyn SlackFeedClient>>` — - Empty result writes nothing and reports `no-new-items`.
