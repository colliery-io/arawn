---
id: extract-arawn-tool-interface-crate
level: task
...
# Extract arawn-tool interface crate from arawn-engine
...
- [ ] `arawn-tui` no longer depends on `arawn-engine` (uses `arawn-service` + `arawn-tool`)
- [ ] `Tool::category() -> ToolCategory` method added to replace string-based `tool_category()`
- [ ] All tests pass, no functionality changes
- [ ] Workspace Cargo.toml updated

## Implementation Notes
- Move types incrementally: start with `Tool` trait + `ToolOutput`, verify compilation, then `ToolRegistry`, then `ToolContext`
- `ToolContext` may need some fields moved to a builder or reduced interface
