...
fn generate_filesystem_rules(profile: &mut String, config: &FilesystemConfig) {
    ...
    // Note: file-write-unlink is covered by (deny default) at the top.
    // Allowed write paths already get (allow file-write* (subpath ...))
    // which includes unlink. No blanket deny needed here.
}
