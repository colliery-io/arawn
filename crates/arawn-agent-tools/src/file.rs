...
/// bypass attempt.
fn reject_traversal(path: &Path) -> std::result::Result<(), arawn_agent::AgentError> {
    ...
    fn resolve_path(&self, path: &str) -> Result<std::path::PathBuf> {
        ...
        if let Some(ref base) = self.base_dir {
            let base_path = Path::new(base).canonicalize().map_err(|e| {
                arawn_agent::AgentError::Tool(format!("Invalid base directory: {}", e))
            })?;
            ...
            let canonical = full_path.canonicalize().map_err(|e| {
                arawn_agent::AgentError::Tool(format!("Cannot resolve path: {}", e))
            })?;

            if !canonical.starts_with(&base_path) {
                return Err(arawn_agent::AgentError::Tool(
        ...
        if let Some(ref base) = self.base_dir {
                ...
                && parent.exists()
            {
                let canonical_parent = parent.canonicalize().map_err(|e| {
                    arawn_agent::AgentError::Tool(format!("Cannot resolve parent path: {}", e))
                })?;
                ...
                let file_name = full_path.file_name().ok_or_else(|| {
                    arawn_agent::AgentError::Tool("Invalid file path".to_string())
                })?;
            ...
            if normalized.starts_with(&base_path) {
                ...
                Err(arawn_agent::AgentError::Tool(
                    "Path is outside allowed directory".to_string(),
                ))
            }
        } else {
