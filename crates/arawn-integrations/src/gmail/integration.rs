...
/// Thin handle so the auth adapter (in [`super::client`]) can write
/// refreshed tokens back without sharing a `TokenStore` borrow across
/// `await` points. Cheap to construct — opens a fresh `TokenStore` per call.
///
/// Public because it appears in the signature of `client::build_gmail_hub`,
/// which is a public helper. The struct itself only carries a path; nothing
/// secret leaks via visibility.
pub struct TokenStoreHandle {
    pub(super) data_dir: PathBuf,
}

impl TokenStoreHandle {
    pub(super) fn save_token(&self, token: &arawn_auth::Token) -> Result<(), IntegrationError> {
